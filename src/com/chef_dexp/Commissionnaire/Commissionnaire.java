...
public class Commissionnaire extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		out.println("<title>List of the commission agent</title>");
		...
		out.println("<h1>Here is the list of commission agent </h1>");
		out.println("<form name=\"f\" action=\"Commissionnaire\">");

		out.println("<fieldset style=\"width: 500px\">");
		out.println("<legend>Choose name</legend>");
		out.println("<label>commision agent</label> ");
		...
		resultat = connexion.getData("SELECT * FROM commissionnaire WHERE codecom="
				+ request.getParameter("commissionnaire") + " ORDER BY nom");

		for (int i = 0; i < resultat.size(); ++i) {
			resultat_final = (Vector) resultat.elementAt(i);

			out.println("<tr><th>code of the comission agent</th><td>" + resultat_final.elementAt(0) + "</td></tr>");
			out.println("<tr style=\"background-color:#DAFAC9;\"><th>Name of the comission agent</th><td>"
					+ resultat_final.elementAt(1) + "</td></tr>");
			out.println("<tr><th>Societé</th><td>" + resultat_final.elementAt(2) + "</td></tr>");
			out.println("<tr style=\"background-color:#DAFAC9;\"><th>Adress</th><td>" + resultat_final.elementAt(3)
					+ "</td></tr>");
			out.println("<tr><th>Email</th><td>" + resultat_final.elementAt(4) + "</td></tr>");
			out.println("<tr style=\"background-color:#DAFAC9;\"><th>Numero de Fax</th><td>"
					+ resultat_final.elementAt(5) + "</td></tr>");
			out.println("<tr><th>Phone number</th><td>" + resultat_final.elementAt(6) + "</td></tr>");
			out.println("<tr style=\"background-color:#DAFAC9;\"><th>Patente</th><td>" + resultat_final.elementAt(8)
					+ "</td></tr>");
			out.println(
					"<tr><th>Informations</th><td><textarea name=Acheteur  width=100% disabled style=\"height:91px;width:400px;margin-bottom: 0px\">"
							+ resultat_final.elementAt(7) + "</textarea></td></tr>");
